...
func (b *Blueprint) Print() {
	...
	for i, _ := range b.transitions {
		if i == 0 {
			builder.WriteString(fmt.Sprintf("%v -> %v", b.transitions[i].from, b.transitions[i].to))
		} else {
			builder.WriteString(fmt.Sprintf(" -> %v", b.transitions[i].to))
		}
	}
	builder.WriteString("\n")
